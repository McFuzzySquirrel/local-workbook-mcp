...
```json
{
  ...
  "pivot_table": "string (optional)",
  "include_filters": "boolean (optional, default: true)",
  "max_rows": "integer (optional, default: 100, max: 500)"
}
```
...
- **pivot_table** (optional): Specific pivot table name to analyze. If omitted, all pivot tables in the worksheet are analyzed
- **include_filters** (optional): Whether to include filter field information in the response (default: true)
- **max_rows** (optional): Maximum number of data rows to extract from the pivot table (default: 100, max: 500)
...
### Contracts (ExcelMcp.Contracts)

- **PivotTableArguments**: Input — `Worksheet`, `PivotTable`, `IncludeFilters`, `MaxRows`
- **PivotTableResult**: Container — `IReadOnlyList<PivotTableInfo>`
- **PivotTableInfo**: Full structure — `Name`, `WorksheetName`, `SourceWorksheet`, `SourceRange`, `RowFields`, `ColumnFields`, `DataFields`, `FilterFields`, `Data`
- **PivotFieldInfo**: Field config — `Name`, `SourceName`, `Function`
- **PivotDataRow**: `Values (IReadOnlyDictionary<string, string?>)`
- **PivotTableMetadata**: Lightweight — `Name`, `WorksheetName`, `SourceRange`, `RowFieldCount`, `ColumnFieldCount`, `DataFieldCount`
