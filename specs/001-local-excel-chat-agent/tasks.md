...
**Prerequisites**: Sample workbook created at `test-data/sample-workbook.xlsx` (generated by `scripts/create-sample-workbook.ps1`)

**Setup Required for Manual Testing**:
1. Start local LLM server (LM Studio with phi-4-mini-reasoning model on http://localhost:1234)
2. Configure `appsettings.Development.json` with correct LLM endpoint
